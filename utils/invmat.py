...
def batch_inverse(x:torch.Tensor):
    """ M(n) -> M(n); x -> x^-1 """
    batch_size, h, w = x.size()
    assert h == w, 'inverse operation allows squares only'
    y = torch.zeros_like(x)
    for i in range(batch_size):
        y[i, :, :] = x[i, :, :].inverse()
    return y

def batch_inverse_dx(y:torch.Tensor):
    """ backward """
    # Let y(x) = x^-1.
    # compute dy
    #   dy = dy(j,k)
...
def batch_pinv_dx(x:torch.Tensor):
...
def batch_inv(x: torch.Tensor):
    """batch inverse
    using torch.linalg.solve to accelerate (computation on GPU)
    Args:
        x (torch.Tensor): (B,M,M)
    Returns:
        inv_x (torch.Tensor): (B,M,M)
    """
    assert x.size(1) == x.size(2), "batch inv can only compute inverse of square matrix, get size:{}".format(x.size())
    M = x.size(1)  
    valid = torch.det(x)!=0
    x_ = x[valid]
    inv_x = torch.zeros_like(x)
    # if torch.abs(x).min().item() == 0:
    #     x += 1e-6  # jitter for inv operation
    identity = torch.eye(M)[None,...].to(x.device)  # (1,M,N)
    inv_x[valid] = torch.linalg.solve(x_,identity)  # need torch>=1.8
    return inv_x
